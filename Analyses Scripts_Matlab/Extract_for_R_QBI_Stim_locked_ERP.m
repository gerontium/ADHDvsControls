...
%     t=t(find(t==-150):find(t==1500));
    ...
    for trial=1:length(allTrials) % get rid of last trigger?
        ...
        if ismember (allTrials(trial), targcodes(:,1)) % any ITI1 targcode.
            master_matrix_R(current:current+(length(t)-1),5) = 1;
        elseif ismember (allTrials(trial), targcodes(:,2))% any ITI2 targcode.
            master_matrix_R(current:current+(length(t)-1),5) = 2;
        else
            master_matrix_R(current:current+(length(t)-1),5) = 3; % any ITI3 targcode.
        end
        %% 6. Target Side:
        if ismember(allTrials(trial),targcodes(1,:))% any left patch targcode. i.e. left target
            master_matrix_R(current:current+(length(t)-1),6) = 1;
            TargetSide=1;
        else
            master_matrix_R(current:current+(length(t)-1),6) = 2; %right target
            TargetSide=2;
        end
        %% 7. Accuracy:
        master_matrix_R(current:current+(length(t)-1),7) = allrespLR(trial); %1=correct 0=wrong
        %% 8. Reaction time (RT):
        master_matrix_R(current:current+(length(t)-1),8)=allRT(trial)*1000/fs;

        %% 9. Blink post target (-100 to 100ms after response):
        master_matrix_R(current:current+(length(t)-1),9) = Blinkneg100_100PR(trial);
        %% 10. Blink post target (-100 to 450ms)
        master_matrix_R(current:current+(length(t)-1),10)= Blinkneg100_450(trial);

        %% 11. Left Fixation Break post target (-100 to 100ms after response):
        master_matrix_R(current:current+(length(t)-1),11)=LeftFixBreakneg100_100PR(trial);
        %% 12. Left Fixation Break post target (-100 to 450ms):
        master_matrix_R(current:current+(length(t)-1),12)=LeftFixBreakneg100_450(trial);
        %% 13. Right Fixation Break post target (-100 to 100ms after response):
        master_matrix_R(current:current+(length(t)-1),13)=RightFixBreakneg100_100PR(trial);
        %% 14. Right Fixation Break post target (-100 to 450ms):
        master_matrix_R(current:current+(length(t)-1),14)=RightFixBreakneg100_450(trial);
        %% 15. Left AND Right Fixation Break post target (-100 to 100ms after response):
        master_matrix_R(current:current+(length(t)-1),15)=BothFixBreakneg100_100PR(trial);
        %% 16. Left AND Right Fixation Break post target (-100 to 450ms):
        master_matrix_R(current:current+(length(t)-1),16)=BothFixBreakneg100_450(trial);
        %% 17. Artefact during post-target window (-100ms to 100ms after response):
        master_matrix_R(current:current+(length(t)-1),17)= artifact_BLintTo100msPostResponse_n(trial);
        %% 18. Rejected trial
        master_matrix_R(current:current+(length(t)-1),18)=rejected_trial_n(trial);
        %% 19. Artefact during post-target window (-100ms to 450ms):
        master_matrix_R(current:current+(length(t)-1),19)=artifact_BLintTo450ms_n(trial);
        %% 20. Pupil Diameter:
        master_matrix_R(current:current+(length(t)-1),20)=Pupil(:,trial);
        %% 21. CPP:
        master_matrix_R(current:current+(length(t)-1),21)=erp(ch_CPP,:, trial);
        %% 22. N2c:
        ch_LR = [23;60];
        ch_N2c = [60;23];
        master_matrix_R(current:current+(length(t)-1),22)=erp(ch_N2c(TargetSide,:),:,trial);
        %% 23. N2i: 
        master_matrix_R(current:current+(length(t)-1),23)=erp(ch_LR(TargetSide,:),:,trial);
        %% 24. Time:
         master_matrix_R(current:current+(length(t)-1),24)=t;
