...
Accuracy_checker_Hemifield <- ddply(data, c("ID", "Hemifield"), summarise,
               Hits  = sum(Accuracy=="Hit"),
               Misses = sum(Accuracy=="Miss"))
Accuracy_checker_Hemifield$Total=Accuracy_checker_Hemifield$Hits+Accuracy_checker_Hemifield$Misses
Accuracy_checker_Hemifield$Accuracy=(Accuracy_checker_Hemifield$Hits/Accuracy_checker_Hemifield$Total)*100
#Bring Target-Hemifield up into wide format
Accuracy_checker_Hemifield <- dcast(Accuracy_checker_Hemifield, ID  ~ Hemifield, value.var="Accuracy")
Accuracy_checker_Hemifield<-rename(Accuracy_checker_Hemifield, c("Left"="Accuracy_Left", "Right"="Accuracy_Right"))
...
#Accuracy_asym: Negitive is better accuracy for left targets, positive is better accuracy for right targets 
Accuracy_checker$Accuracy_Asym<-(Accuracy_checker$Accuracy_Right - Accuracy_checker$Accuracy_Left)/(Accuracy_checker$Accuracy_Right + Accuracy_checker$Accuracy_Left)
...
data_participant_level$Hand_SR_B <- factor(data_participant_level$Hand_SR_B)
data_participant_level$DAT1_3UTR<- factor(data_participant_level$DAT1_3_Group)
data_participant_level$DAT1_i8<- factor(data_participant_level$DAT1_i8_Group)

#Rename factor Levels:
data_participant_level$Group <- revalue(data_participant_level$Group, c("1"="ADHD", "2"="Control"))
data_participant_level$Gender <- revalue(data_participant_level$Gender, c("1"="Male", "2"="Female"))
data_participant_level$Handedness_SelfReport <-  revalue(data_participant_level$Hand_SR_B, c("1"="Left", "2"="Right"))
data_participant_level$DAT1_3UTR<- revalue(data_participant_level$DAT1_3UTR, c("1"="non10repeat_homozygous", "2"="10repeat_homozygous"))
data_participant_level$DAT1_i8<- revalue(data_participant_level$DAT1_i8, c("1"="non6repeat_homozygous", "2"="6repeat_homozygous"))
...
data_participant_level$IQ<-data_participant_level$eFSIQ_B
data_participant_level$Handedness_Edinburgh<-data_participant_level$HQ_B
data_participant_level$Reading <-data_participant_level$Read_Standard_B #WRAT Reading standard score (at time B)
data_participant_level$Spelling<-data_participant_level$Spell_Standard_B # WRAT Spelling standard score (at time B)

data_participant_level$Accuracy_Asym.Z<-scale(data_participant_level$Accuracy_Asym)


max(abs(scale(data_participant_level$Accuracy[data_participant_level$Group=="ADHD"])))
max(abs(scale(data_participant_level$Accuracy_Left[data_participant_level$Group=="ADHD"])))
max(abs(scale(data_participant_level$Accuracy_Right[data_participant_level$Group=="ADHD"])))
max(abs(scale(data_participant_level$Accuracy_Asym[data_participant_level$Group=="ADHD"])))

max(abs(scale(data_participant_level$Accuracy[data_participant_level$Group=="Control"])))
max(abs(scale(data_participant_level$Accuracy_Left[data_participant_level$Group=="Control"])))
max(abs(scale(data_participant_level$Accuracy_Right[data_participant_level$Group=="Control"])))
max(abs(scale(data_participant_level$Accuracy_Asym[data_participant_level$Group=="Control"])))

t.test(data_participant_level$Accuracy_Asym~data_participant_level$Group)

t.test(data_participant_level$Accuracy_Asym[data_participant_level$Group=="ADHD"],mu=0)

t.test(data_participant_level$Accuracy_Asym[data_participant_level$Group=="Control"],mu=0)
...
#....and also excludes C194 because this participant only had 57 valid trials for EEG (less than 50 valid trials for EEG)
######################################################
...
ezANOVA(
    ...
    , wid = .(ID)
    , within = .(Hemifield)
    , within_full = .(Hemifield, ITI, Trial)
    , between = (Group)
    , type = 3
    , detailed = F
)
...
#use nlme to get t and df for the Hemifield contrast:
require(nlme)
summary(lme(log(RT) ~ Hemifield, random = ~1|ID/ITI/Hemifield/Trial, data = data2[data2$Group=="Control",], na.action = na.omit))

rcontrast<-function(t, df)
{r<-sqrt(t^2/(t^2 + df))
print(paste("r = ", r))
}
rcontrast(1.89724,  92)
...
dan1<-lmer(log(RT) ~ 1 + (1 | ID)+(1|ITI) + (1|Trial), data = data2[data2$Group=="Control",], REML=FALSE, na.action = na.omit)
dan2<-lmer(log(RT) ~ Hemifield + (1 | ID)+(1|ITI) + (1|Hemifield)+ (1|Trial), data = data2[data2$Group=="Control",], REML=FALSE, na.action = na.omit)

1-var(residuals(dan1))/(var(model.response(model.frame(dan1))))

1-var(residuals(dan2))/(var(model.response(model.frame(dan2))))

r_squared_change<-((1-var(residuals(dan2))/(var(model.response(model.frame(dan2)))))  -  
    (1-var(residuals(dan1))/(var(model.response(model.frame(dan1))))))
...
##Try the old ANOVA approach 
ezANOVA(
    data = DF
    , dv = .(N2)
    , wid = .(ID)
    , within = .(Hemifield, Hemisphere)
    , within_full = .(Hemifield, Hemisphere, ITI, Trial)
    , between = .(Group)
...
###Still should try some machine learning to classify into ADHD vs Controls.
